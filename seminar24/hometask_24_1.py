...
class Student:    
    def __init__(self, name, subjects_file) -> None:
        self.name = name
        self.subjects = {subject: {'grade': None, 'test_score': None} for subject in self.load_subjects(subjects_file)}        

    def __setattr__(self, name: str, value):        
        if name == 'name' and not(name.isalpha() and value[0].istitle()):
    ...
    def __str__(self) -> str:                        
        return f"Студент: {self.name}\nПредметы: {', '.join(key for key in self.subjects if self.subjects[key]['grade'] != None and self.subjects[key]['test_score'] != None)}"

    def load_subjects(self, subjects_file) -> list:
        subjects_list = []
        with open(subjects_file, 'r', newline='', encoding='utf-8') as fcsv:                
    ...
    def get_average_grade(self):
        total_grade = sum(values['grade'] for values in self.subjects.values() if values['grade'] != None)
        count_subjects_with_grades = sum(values['grade'] != None for values in self.subjects.values())
        return total_grade / count_subjects_with_grades



if __name__ == '__main__':
    student = Student("Иван Иванов", "seminar24/subjects.csv")  
